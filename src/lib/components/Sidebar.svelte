...
<aside class="w-64 bg-card text-card-foreground border-r flex flex-col">
    ...
    {#each menuItems as item}
      <a 
        ...
        {#if item.pillCount !== undefined}
          <span class="ml-auto bg-orange-600 text-white text-xs font-semibold px-2 py-1 rounded-full">{item.pillCount}</span>
        {/if}     
      </a>
      {#if item.subItems}
  ...
  <div class="mt-auto ml-3 mb-2 text-sm">
    <a href="/manage/ai" class="flex items-center px-4 py-2 hover:bg-accent text-blue-500"><WandSparkles class="mr-2" size={20} />
      AI-Compliance Check
      <span class="ml-auto text-sm text-blue-400">New!</span>
    </a>
